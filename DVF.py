...
PERSONAS = [
    ...
    {"id": "p06", "name": "Health-Conscious Pro","prior": {"des": 3.9, "via": 3.3, "fea": 3.1}},
...
with st.sidebar:
    ...
    selected_ids   = st.multiselect("Select Personas", [p["name"] for p in PERSONAS],
                                    default=[p["name"] for p in PERSONAS])
    sigma          = st.slider("Variance σ", min_value=0.05, max_value=0.50,
                               step=0.05, value=0.15)
    w_prior        = st.select_slider("Prior weight", options=[0.0,0.25,0.5,0.75,1.0],
                                      value=0.3)
    st.divider()
...
# Tabs
# ------------------------------
tabs = st.tabs(["DVF Map", "Persona Details", "Variance Matrix",
                "Summary", "Compare", "Export"])
tab_map, tab_det, tab_var, tab_sum, tab_cmp, tab_exp = tabs
...
with tab_map:
    ...
    for p, resp, coord in zip(personas, responses, coords):
        base = compute_dvf({k:v for k,v in resp.items() if k!="persona"}, p["prior"], w_prior)
        dvf = {d: add_variance(s, sigma) for d,s in base.items()}
        rows.append({**{"id": p["id"], "name": p["name"], **dvf, **coord}})
    df = pd.DataFrame(rows)
    # plot
    fig = px.scatter(df, x="via", y="des",
                     size="fea", color="fea",
                     color_continuous_scale="RdYlGn_r",
                     range_color=[1,5], size_max=60, opacity=0.9,
    ...
    fig.update_layout(xaxis_title="Viability",
